...
void CustomPlugin::_addAnalyzeToolsEntry(const QString &title, const char *qmlFile, const char *iconFile)
{
    Q_CHECK_PTR(qmlFile);
    // 'this' instance will take ownership on the QmlComponentInfo instance
    _customAnalyzeToolsList.append(QVariant::fromValue(
        new QmlComponentInfo(title,
                             QUrl::fromUserInput(qmlFile),
                             iconFile == nullptr ? QUrl() : QUrl::fromUserInput(iconFile),
                             this)));
...
CustomPlugin::settingsPages()
{
    if(_customSettingsList.isEmpty()) {
        ...
        _addSettingsEntry(tr("Debug"),   "qrc:/qml/DebugWindow.qml");
...
{
    if(_customAnalyzeToolsList.isEmpty()){
        _addAnalyzeToolsEntry(tr("Log Download"), "qrc:/qml/LogDownloadPage.qml", "qrc:/qmlimages/LogDownloadIcon");
        _addAnalyzeToolsEntry(tr("GeoTag Images"), "qrc:/qml/GeoTagPage.qml", "qrc:/qmlimages/GeoTagIcon");
...
QQmlApplicationEngine* CustomPlugin::createQmlApplicationEngine(QObject* parent)
{
    ...
    qmlEngine->rootContext()->setContextProperty("qgcApplication", qgcApp());
    qmlEngine->rootContext()->setContextProperty("opCls", opCls);
